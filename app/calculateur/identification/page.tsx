...
import { STORAGE_KEYS, initialClient } from '@/lib/store/projetStore';
...
export default function IdentificationPage() {
  ...
  useEffect(() => {
    if (isLoaded) {
      try {
        localStorage.setItem(STORAGE_KEYS.CLIENT, JSON.stringify(formData));
      } catch (error) {
  ...
  const validateField = useCallback((name: keyof Client, value: string): string => {
    switch (name) {
      case 'email':
  ...
  const handleBlur = (name: keyof Client) => {
  ...
  const validateForm = (): boolean => {
    ...
    const requiredFields: (keyof Client)[] = ['nom', 'prenom', 'email', 'telephone'];
    ...
    const allTouched: Partial<Record<keyof Client, boolean>> = {};
    requiredFields.forEach(field => {
        ...
        <div className="flex gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-primary-600/80 mt-1 leading-relaxed">
              Vos informations sont utilisées uniquement pour le traitement de votre commande
